...
function polygon2latlngs (pg) {
	if (pg == null) return null;
	var datas = pg.substring(pg.indexOf("((") + 2, pg.length-2).split(' ');
	var latlngs = [];
	for (var i=0; i<datas.length; i+=2) {
			latlngs.push( new LagLngPoint(parseFloat(datas[i+1]), parseFloat(datas[i])) );
	}
	return latlngs;
}

function sridlocation2laglng (sridLocation) {
	if (sridLocation == null) return null;
	var datas = sridLocation.substring(sridLocation.indexOf("(")+1, sridLocation.length-1).split(" ");
	return new LagLngPoint(parseFloat(datas[1]), parseFloat(datas[0]));
}

function lineString2laglngs (linestr) {
	if (linestr == null) return null;
	var datas = linestr.substring(linestr.indexOf("(") + 1, linestr.length-1).split(' ');
	var latlngs = [];
	for (var i=0; i<datas.length; i+=2) {
			latlngs.push( new LagLngPoint(parseFloat(datas[i+1]), parseFloat(datas[i])) );
	}
...
export {polygon2latlngs, sridlocation2laglng, lineString2laglngs, LagLngPoint, bd09ToGcj02, gcj02ToBd09, wgs84ToGcj02, gcj02ToWgs84}